...
export default function HRLogInPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch("http://127.0.0.1:8000/hr/login", {
        method: "POST",
      ...
      if (res.ok && data.success && data.token) {
        // Store the JWT in localStorage for later use
        localStorage.setItem("authToken", data.token);
        localStorage.setItem("userRole", data.user.role); // Optional: Store user role
